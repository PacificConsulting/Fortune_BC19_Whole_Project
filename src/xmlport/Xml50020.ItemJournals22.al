    ...
    schema
    {
        textelement(Root)
        {
            tableelement("Item Journal Line"; "Item Journal Line")
            {
                ...
                fieldelement(Quantity; "Item Journal Line"."Qty. (Calculated)")
                {

                    trigger OnAfterAssignField();
                    begin
                        //"Item Journal Line"."Phys. Inventory" := TRUE;
                        "Item Journal Line".VALIDATE("Phys. Inventory", TRUE);
                        //"Item Journal Line".VALIDATE("Qty. (Phys. Inventory)","Item Journal Line"."Qty. (Calculated)");
                    end;
                }
                fieldelement(Quantity2; "Item Journal Line"."Qty. (Phys. Inventory)")
                {
                }
