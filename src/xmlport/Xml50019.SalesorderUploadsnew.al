xmlport 50019 "Sales order Uploads - new"
{
    // version CCIT-Vikas
    ...
    schema
    {
        textelement(Root)
        {
            tableelement("Sales Header"; "Sales Header")
            {
                ...
                textelement(sellname2)
                {
                }
                textelement(customerorderno)
                {
                }
                ...
                begin
                    ...
                    IF NewNo <> PrevNo THEN BEGIN
                        ...
                        SalesHeader.VALIDATE("Sell-to Customer Name 2", sellname2);
                        SalesHeader.VALIDATE("External Document No.", customerorderno);
                    ...
                    EVALUATE(Qty3, qtyinpc);
                    SalesLine.VALIDATE("Main Quantity in PCS", Qty3);
                    //EVALUATE(UnitPrice,UnitPriceExTax);
                    //SalesLine.VALIDATE("Unit Price",UnitPrice);
                    //SalesLine."Special Price" := SpecialPrice;
                    EVALUATE(linediscountPER, linediscount);
                    SalesLine.VALIDATE("Line Discount %", linediscountPER);
                    SalesLine.INSERT();
