report 50094 "Inventory Planning HO - new1"
{
    // version RDK not in use
    ...
    begin
        ...
        *///160919
        ...
        IF RecLoc.FINDSET THEN
            REPEAT
            //  ExcelBuf.AddColumn(RecLoc.Code,FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text); //160919
            //  ExcelBuf.AddColumn('',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);//160919
            UNTIL RecLoc.NEXT = 0;
        ExcelBuf.AddColumn('Total PAN INDIA', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text);
        //ExcelBuf.AddColumn('',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);//160919

        //ExcelBuf.AddColumn('TotalSales In KGs',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);//Total Sales //Excel column CA //160919

        ExcelBuf.AddColumn('DAS In KGS', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text); //Excel column CB
        ExcelBuf.AddColumn('Days Stock', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text); //Excel column CC
        ExcelBuf.AddColumn('Today Date', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text); //Excel column CD
        ExcelBuf.AddColumn('Stock will Last till', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text); //Excel column CE

        //>>
        ExcelBuf.AddColumn('Intransit 1', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text);
        //ExcelBuf.AddColumn('',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);//21-09-2019
        //ExcelBuf.AddColumn('',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);//21-09-2019
        ...
        *///160919
        ...
        IF RecLocation.FINDSET THEN
            REPEAT
            //  ExcelBuf.AddColumn('IOH in KGS',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);//160919
            //  ExcelBuf.AddColumn('IOH in PCS',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);//160919
            UNTIL RecLocation.NEXT = 0;
        ExcelBuf.AddColumn('IOH in KGS', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text);
        //ExcelBuf.AddColumn('IOH in PCS',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);//160919

        //ExcelBuf.AddColumn('',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);//Total Sales //160919
        ...
        //>> Intrasit -4
        ExcelBuf.AddColumn('PO No', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text);
        //ExcelBuf.AddColumn('PO Date',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);//160919
        ExcelBuf.AddColumn('Qty In KG', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn('Saleable Date', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text); //25-05-2019
        ExcelBuf.AddColumn('Days', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text); //25-05-2019
        ExcelBuf.AddColumn('Stocks Last Till', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text); //25-05-2019
        ...
        *///16092019
        ...
        IF RecLoc.FINDFIRST THEN
            REPEAT
                ...
                RecILE2.SETRANGE(RecILE2."Posting Date", 99990101D, TODAY); //010199D
            ...
            //ExcelBuf.AddColumn(LocQtyPCS,FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Number);//160919
            //ExcelBuf.AddColumn(LocQtyKG,FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Number);//160919

            UNTIL RecLoc.NEXT = 0;
        // Line wise Total for Total PAN INDIA
        ExcelBuf.AddColumn(TotalLocQtyPCS, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Number);
        //ExcelBuf.AddColumn(TotalLocQtyKG,FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Number);//160919
        ...
        IF DAS_Filter = DAS_Filter::"1Month" THEN BEGIN
            ...
            ExcelBuf.AddColumn(TODAY, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date); //Excel column CD
                                                                                                       //ExcelBuf.AddColumn(DateIOHWLLast,FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Date); //Excel column CE // 300919
        END
        ELSE
            IF DAS_Filter = DAS_Filter::"2Month" THEN BEGIN
                //IF ItemTotal1 <> 0 THEN
        ...
        IF PurchLine.FINDFIRST THEN BEGIN
            ...
            IF DASValue2 <> 0 THEN BEGIN
                ...
                IF DateIOHWLLast < PurchHeader."ETA - Availability for Sale" THEN // rdk16092019
                    StockLastTill := PurchHeader."ETA - Availability for Sale" + DASQty
                ELSE
                    StockLastTill += DASQty;
            END;
        END;

        IF SaleableDate < PurchHeader."ETA - Availability for Sale" THEN BEGIN
            // ExcelBuf.AddColumn('YES',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);
            ExcelBuf.AddColumn(SaleableDate, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date);// 280919 //AddColumnwithColor //PCPL/MIG/NSW Filed not Exist in BC18 Function change
        END
        ELSE
            ExcelBuf.AddColumn(SaleableDate, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date); //Excel column CE // 300919
        ...
        IF PurchLine.FINDFIRST THEN
            REPEAT
                ...
                i += 1;
                IF i <= 5 THEN BEGIN
                    //ExcelBuf.AddColumn(PurchLine."Document No.",FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Text); //21-09-2019
                    ExcelBuf.AddColumn(PurchHeader."Vendor Order No.", FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);// 21-09-2019
                    ...
                    ELSE
                        IF DASValue2 <> 0 THEN BEGIN
                            ...
                            IF SaleableDate < PurchHeader."ETA - Availability for Sale" THEN BEGIN
                                // ExcelBuf.AddColumn('YES',FALSE,'',TRUE,FALSE,TRUE,'',ExcelBuf."Cell Type"::Text);
                                ExcelBuf.AddColumn(StockLastTill, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date);// 280919 ////PCPL/MIG/NSW Filed not Exist in BC18 Function Chnage
                            END
                            ELSE BEGIN
                        ...
                        ELSE BEGIN
                            ...
                            ExcelBuf.AddColumn(0, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date); //25-05-2019
                                                                                                                   //ExcelBuf.AddColumn('',FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Text);//rdk160919
                        END;
    ...
    begin
        ...
        IF RecLoc.FINDFIRST THEN
            ...
            // Location wise total
            // ExcelBuf.AddColumn(LocTotalPCS,FALSE,'',TRUE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Number);//160919
            // ExcelBuf.AddColumn(LocTotalKG,FALSE,'',TRUE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Number);//160919
