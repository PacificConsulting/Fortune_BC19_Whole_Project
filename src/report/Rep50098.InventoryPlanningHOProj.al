report 50098 "Inventory Planning HO - Proj"
{
    // version RDK in use
        ...
        layout
        {
            area(content)
            {
                group("Request Fields")
                {
                    ...
                    field("Projection Period"; "Projection Period")
                    {
                    }
                }
            }
    ...
    begin
        ...
        IF FORMAT("Projection Period") <> '' THEN BEGIN
            Proj_From_date := CALCDATE('-CM', Date);
            Proj_To_date := CALCDATE("Projection Period", Proj_From_date);
        END;
    end;
    ...
    var
        ...
        StockLastTill: Date;
        DateIOHWLLast: Date;
        "Projection Period": DateFormula;
        Proj_From_date: Date;
        Proj_To_date: Date;
        ItemBudgetEntry: Record 7134;
        TotalProj: Decimal;
        GrandTotalProj: Decimal;
    ...
    begin
        ExcelBuf.NewRow;
        ExcelBuf.AddColumn('INVENTORY PLANNING PURCHASE HO (PROJECTIONS)', FALSE, '', TRUE, TRUE, TRUE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.NewRow;
        ExcelBuf.NewRow;
        ...
        ExcelBuf.NewRow;
        ExcelBuf.AddColumn('Date Range for Projection:' + FORMAT(Proj_From_date) + '  To  ' + FORMAT(Proj_To_date), FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.NewRow;
        ...
        ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn('Total Sales', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text);//Total Sales //Excel column CA
        ExcelBuf.AddColumn('Projections In KGs', FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text);//Total Sales //Excel column CA
        ExcelBuf.AddColumn('DAS', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text); //Excel column CB
        ExcelBuf.AddColumn('Projections Per Day', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text); //Excel column CB
    ...
    begin
        ...
        IF RecLoc.FINDFIRST THEN
            REPEAT
                ...
                RecILE2.SETRANGE(RecILE2."Posting Date", 99990101D, TODAY); //PCPL/MIG/NSW Filed not Exist in BC18
        ...
        ItemBudgetEntry.SETRANGE(Date, Proj_From_date, Proj_To_date);
        ItemBudgetEntry.CALCSUMS(Quantity);
        TotalProj += ItemBudgetEntry.Quantity;
        GrandTotalProj += ABS(TotalProj); //Grand total for Projections

        ExcelBuf.AddColumn(ABS(TotalProj), FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Number);//Total Proj
        ...
        ELSE
            IF DAS_Filter = DAS_Filter::"2Month" THEN BEGIN
                ...
                ExcelBuf.AddColumn(DASValue2, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Number); //Excel column CC

                DASValue2 := ROUND(ABS(TotalProj) / 52, 0.1, '='); //
                IF DASValue2 <> 0 THEN
                    DAYS2 := ROUND(TotalLocQtyPCS / DASValue2, 1, '=');

                DateIOHWLLast := TODAY + DAYS2;
                // ExcelBuf.AddColumn(ROUND(TotalProj/(Proj_To_date-Proj_From_date),0.1,'='),FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Number);

                ExcelBuf.AddColumn(DASValue2, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Number);//col cd
