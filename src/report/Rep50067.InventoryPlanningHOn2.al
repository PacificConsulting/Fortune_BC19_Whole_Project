report 50067 "Inventory Planning HO - n2"
{
    // version RDK in use
        ...
        dataitem("Item Vendor"; "Item Vendor")
        {
            ...
            dataitem(Item; Item)
            {
                ...
                begin
                    ...
                    ItemTotal := 0;
                    ILE.RESET;
                    //ILE.SETCURRENTKEY("Item No.","Entry Type");//CCIT-HG-080519  //PCPL/MIG/NSW Filed not Exist in BC18
    ...
    begin
        ...
        IF DASValue2 = 0 THEN BEGIN
            ExcelBuf.AddColumn(0, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Number); //Excel column CB
            ExcelBuf.AddColumn(0, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Number); //Excel column CC
            ExcelBuf.AddColumn(TODAY, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date); //Excel column CD
        END;
        ...
        IF PurchLine.FINDFIRST THEN BEGIN
            ...
            IF DASValue2 <> 0 THEN BEGIN
                IF SaleableDate < PurchHeader."ETA - Availability for Sale" THEN // rdk16092019
                    ExcelBuf.AddColumn(SaleableDate, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date)// 280919 //PCPL/MIG/NSW Filed not Exist in BC18 Function changes

                ELSE
                    ExcelBuf.AddColumn(SaleableDate, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date); //Excel column CE // 300919
            END
            ELSE
                ExcelBuf.AddColumn(TODAY, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date); //Excel column CE
        END
        ELSE BEGIN
            IF DASValue2 <> 0 THEN
                ExcelBuf.AddColumn(SaleableDate, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date)
            ELSE
                ExcelBuf.AddColumn(TODAY, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date); //Excel column CE
        END;
        //IF DASValue2 = 0 THEN
        //  ExcelBuf.AddColumn('' ,FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Date); //Excel column CE // 300919
        ...
        IF SaleableDate < PurchHeader."ETA - Availability for Sale" THEN
          BEGIN
            ...
            ExcelBuf.AddColumnwithColor(SaleableDate,FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Date,1,TRUE);// 280919
          END
        ELSE
            ExcelBuf.AddColumn(SaleableDate ,FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Date); //Excel column CE // 300919
         */
        ...
        StockLastTill_Tmp := SaleableDate;
            ...
            REPEAT
                ...
                IF i <= 5 THEN BEGIN
                    ...
                    IF DASValue2 <> 0 THEN BEGIN
                        ...
                        IF SaleableDate < PurchHeader."ETA - Availability for Sale" THEN // rdk16092019
                            StockLastTill_Tmp := PurchHeader."ETA - Availability for Sale" + DASQty
                        ELSE
                            StockLastTill_Tmp += DASQty;
                    END;
                    // +
                    IF PurchLine.NEXT <> 0 THEN BEGIN
                        IF PurchHeader.GET(PurchLine."Document Type", PurchLine."Document No.") THEN;
                        IF DASValue2 <> 0 THEN BEGIN
                            IF StockLastTill_Tmp < PurchHeader."ETA - Availability for Sale" THEN // rdk16092019
                                StkOut := TRUE
                            ELSE
                                StkOut := FALSE;
                        END;
                    ...
                    ELSE
                        IF DASValue2 <> 0 THEN BEGIN
                            ...
                            // rdk 30-08-2019 -
                            IF StkOut THEN
                                ExcelBuf.AddColumn(StockLastTill, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date)// 280919 //PCPL/MIG/NSW Filed not Exist in BC18 Func change AddColumnwithColor
                            ELSE
                                ExcelBuf.AddColumn(StockLastTill, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Date); //25-05-2019

                            SaleableDate := StockLastTill;
                            StockLastTill_Tmp := StockLastTill;
                            // rdk 30-08-2019 +
                        END
                        ELSE BEGIN
