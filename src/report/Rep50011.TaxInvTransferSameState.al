report 50011 "Tax Inv Transfer - Same State"
{
    // version RDK,CITS_RS

    DefaultLayout = RDLC;
    RDLCLayout = 'src/reportlayout/Tax Inv Transfer - Same State.rdl';
    ApplicationArea = all;
    ...
    dataset
    {
        dataitem("Transfer Shipment Header"; "Transfer Shipment Header")
        {
            ...
            column(RepHeading; RepHeading)
            {
            }
            column(MFGDate; MFGDate)
            {
            }
            column(PrintGSTDetails; PrintGSTDetails)
            {
            }
            column(GSTHeading; GSTHeading)
            {
            }
            column(TotalCGSTSGST; TotalCGSTSGST)
            {
            }
            dataitem("Transfer Shipment Line"; "Transfer Shipment Line")
            {
                ...
                begin
                    ...
                    TotalCGSTSGST := TotalCGST + TotalSGST;
                    TotalIGST += IGSTAmt;
                    ...
                    AmtInWords.FormatNoText(AmountinWords, ROUND(TotalAmount1), '');
            ...
            begin
                ...
                "Transfer Shipment Header".CALCFIELDS("QR Code");//CITS_RS 080221
                ...
                PrintGSTDetails := FALSE;

                //IF (RecState1.Code = 'MAH') = (RecState2.Code = 'MAH') THEN
                IF (FromStateCode = 'MAH') THEN BEGIN
                    IF (ToStateCode = 'MAH') THEN BEGIN
                        RepHeading := 'Delivery Challan';
                        GSTHeading := 'CGST + SGST';
                        PrintGSTDetails := TRUE;
                    END;
                END
                ELSE BEGIN
                    RepHeading := 'Tax Invoice';
                    GSTHeading := 'IGST';
                    PrintGSTDetails := FALSE;
                END;
            end;
    ...
    var
        ...
        RepHeading: Text[50];
        PrintGSTDetails: Boolean;
        GSTHeading: Text[20];
        TotalCGSTSGST: Decimal;
        Original: Boolean;
        Duplicate: Boolean;
        Triplicate: Boolean;
        Textmsg: Text;
