report 50091 "Sales Reg inv-Cr Mom Range"
{
    // version RDK in use

    DefaultLayout = RDLC;
    RDLCLayout = 'src/reportlayout/Sales Reg inv-Cr Mom Range.rdl';
            ...
            dataitem(SIH; "Sales Invoice Header")
            {
                ...
                dataitem(SIL; "Sales Invoice Line")
                {
                    ...
                    begin
                        ...
                        //IF 301110|301120|301130|301140|301141
                ...
                dataitem(SCML; "Sales Cr.Memo Line")
                {
                    ...
                    begin
                        IF SCML.Quantity = 0 THEN
                            CurrReport.SKIP;
                        //RL
                        IF SCML.Type = SCML.Type::"G/L Account" THEN BEGIN
                            GLAccount.RESET;

                            GLAccount.SETRANGE("No.", SCML."No.");
                            GLAccount.SETRANGE("Sales Account", TRUE);
                            IF NOT GLAccount.FINDFIRST THEN
                                CurrReport.SKIP;
                        END;
    ...
    begin
      ...
      ELSE
      ExcelBuf.AddColumn('',FALSE,'',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Text);*/
        /*IF NOT Rec_Item.GET(Temp_CustItemMom."Code 2") THEN
        ExcelBuf.AddColumn('',FALSE,',FALSE,FALSE,FALSE,'',ExcelBuf."Cell Type"::Text)
        ELSE *///RL
        ExcelBuf.AddColumn(Temp_CustItemMom."Code 2", FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);

        IF Rec_Item.GET(Temp_CustItemMom."Code 2") THEN BEGIN
            ExcelBuf.AddColumn(Rec_Item.Description, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text)
        END;
        IF GLAccount.GET(Temp_CustItemMom."Code 2") THEN BEGIN
            ExcelBuf.AddColumn(GLAccount.Name, FALSE, '', TRUE, FALSE, TRUE, '', ExcelBuf."Cell Type"::Text);
        END;

        IF Rec_Item.GET(Temp_CustItemMom."Code 2") THEN
            ExcelBuf.AddColumn(Rec_Item."Sales Category", FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text)
        ELSE
